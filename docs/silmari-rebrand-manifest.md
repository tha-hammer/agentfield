...
| `git log -1 --pretty=%s (latest commit subject pattern assertion)` | `.` | 0 | Passed: the post-commit fast rerun confirmed the latest local commit subject matched the required Silmari/rebrand pattern. |
| `python3 - <<'PY' (latest commit scope assertion)` | `.` | 0 | Passed: the local verification commits stayed within rebrand scope, touching only `docs/` evidence and the web triggers-page test that asserts rebranded copy. |
...
| `./scripts/check-silmari-rebrand.sh` | `.` | 0 | Passed: the post-commit fast rerun scanned 1336 files, audited 168 changed files, validated 1327 preserved identifier rows, and confirmed skill mirror parity. |
...
| `python3 - <<'PY' (changed-file coverage assertion)` | `.` | 0 | Passed: every changed in-scope rebrand file, including the post-review `triggers-pages.test.tsx` stabilization, is represented in the manifest Audited Files table. |
