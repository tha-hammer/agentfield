...
vi.mock("@/config/navigation", () => ({
  ...
  isNavBranch: (item: MockNavItem) => !!item.children,
}));
...
vi.mock("@/components/ui/sidebar", async () => {
  ...
  return {
    ...
    SidebarMenuButton: ({
      ...
      isActive,
      tooltip,
    }: React.PropsWithChildren<{ isActive?: boolean; tooltip?: string }>) => (
      <div data-active={isActive} title={tooltip}>
        {children}
      </div>
    ),
    SidebarSeparator: () => <hr />,
...
describe("AppSidebar", () => {
  it("renders the logo, navigation, and resource links", () => {
    ...
    expect(screen.getByTitle("Silmari")).toBeInTheDocument();
    expect(screen.getByText("Control Center")).toBeInTheDocument();
    expect(screen.getByTitle("Dashboard")).toBeInTheDocument();
    expect(screen.getByTitle("Runs")).toBeInTheDocument();
    expect(screen.getByText("Resources")).toBeInTheDocument();
    expect(screen.getByText("Silmari Docs")).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Toggle Mode" })).toBeInTheDocument();
  });

  it("marks the active navigation item based on exact path", () => {
    renderWithRouter(["/dashboard"]);
    const dashboardLink = screen.getByTitle("Dashboard").closest('[data-active]');
    const runsLink = screen.getByTitle("Runs").closest('[data-active]');
  ...
  it("renders external resource links with correct attributes", () => {
    renderWithRouter(["/dashboard"]);
    const docLink = screen.getByText("Silmari Docs").closest("a");
