...
- Fixup: mocks/tests missed by the merge conflict resolution

Caught by actually running the test suites (not just syntax/compile
checks) after the merge commit:

- test_harness_provider_opencode.py: one capture_cmd mock
  (test_opencode_project_dir_takes_precedence_over_cwd) never had an
  idle_timeout param at all, so it wasn't matched by the earlier
  idle_timeout->idle_seconds rename pass (which only matched the
  `idle_timeout=None):` pattern). Added idle_seconds=None.
- test_harness_cli.py: 3 idle-watchdog tests were fork-original and
  never touched by upstream's diff, so they had no merge conflict but
  were stale against the resolved _cli.py: still called
  idle_timeout= (now idle_seconds=), asserted the fork's old error
  text ("idle stall"/"total timeout" vs upstream's "made no progress
  for Ns"/"timed out after Ns"), and _FakeProc was missing a .pid
  attribute that the new killpg-based _kill_group() needs (added,
  matching the convention already used in test_run_cli_timeout).
- agent_test.go: TestNew/CallTimeout_defaults_to_15s_when_unset
  asserted upstream's original 15s default, which the merge commit
  deliberately overrode to 10m. Updated the assertion and renamed the
  subtest; updated the CallTimeout field's doc comment to match.

Full suites now green: `uv run pytest tests/` (Python SDK) and
`go test ./...` (Go SDK). Web-client/control-plane suites not yet run.

Co-Authored-By: Claude Sonnet 5 <noreply@anthropic.com> (a811dff)
